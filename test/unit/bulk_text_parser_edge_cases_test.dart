...
''';
    ...
    test(
        'filters parenthetical location sub-headers and footer instruction notes',
        () {
      const input = '''
*02.08.2026*
*9/B GÖREV Listesi*
...
''';
    ...
    test(
        'normalizes team format 3B- to 3/B and parses numbering without dot like 10J.Uzm.Çvş.',
        () {
      const input = '''
...
''';
      final result = BulkTextParser.parse(input);
      expect(result.blocks.single.parsedTimName, '3B');
      expect(result.blocks.single.personnelList.single.rawName,
          'Abdusamed ÖZAĞAÇKAYA');
    });
  });
}
