...
void main() {
  ...
  group('Responsive Layout & Overflow Prevention Tests', () {
    ...
    for (final size in viewports) {
      testWidgets(
        'DashboardScreen has zero layout overflow on viewport ${size.width}x${size.height}',
      ...
      testWidgets(
        'PersonnelManagementScreen has zero layout overflow on viewport ${size.width}x${size.height}',
        (WidgetTester tester) async {
          await setSurfaceSize(tester, size);
          await tester.pumpWidget(
            buildTestableWidget(const PersonnelManagementScreen()),
          );
          await tester.pump(const Duration(milliseconds: 200));
      ...
      testWidgets(
        'TemgundrapScreen has zero layout overflow on viewport ${size.width}x${size.height}',
        (WidgetTester tester) async {
          await setSurfaceSize(tester, size);
          await tester.pumpWidget(
            buildTestableWidget(const TemgundrapScreen()),
          );
          await tester.pump(const Duration(milliseconds: 200));
      ...
      testWidgets(
        'ActivityArchiveScreen has zero layout overflow on viewport ${size.width}x${size.height}',
        (WidgetTester tester) async {
          await setSurfaceSize(tester, size);
          await tester.pumpWidget(
            buildTestableWidget(const ActivityArchiveScreen()),
          );
          await tester.pump(const Duration(milliseconds: 200));

          expect(tester.takeException(), isNull);
          expect(find.byType(ActivityArchiveScreen), findsOneWidget);
          expect(find.byKey(const Key('archive-previous-day')), findsOneWidget);
          expect(find.byKey(const Key('archive-next-day')), findsOneWidget);
          await tester.pumpWidget(Container());
          await tester.pump(const Duration(milliseconds: 200));
        },
      );
      ...
      testWidgets(
        'LoginScreen has zero layout overflow on viewport ${size.width}x${size.height}',
        (WidgetTester tester) async {
          await setSurfaceSize(tester, size);
    ...
    testWidgets(
      'PersonnelFormDialog renders cleanly with 1.5x large text scaling without overflow',
      (WidgetTester tester) async {
        await setSurfaceSize(tester, const Size(360, 800));
        await tester.pumpWidget(
          buildTestableWidget(const PersonnelFormDialog(), textScale: 1.5),
        );
        await tester.pump(const Duration(milliseconds: 200));
